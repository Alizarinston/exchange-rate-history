...
class TestViews(TestCase):
    ...
    def test_insert_record(self):
        ...
        self.assertEqual(before_insert, 0)
        self.assertEqual(resp.status_code, 302)
        self.assertEqual(after_insert, 1)
        self.assertEqual(continuity, '2020-01-09')

    def test_delete_record(self):
        """ Deleting course valid from a specific date """
        before_delete = ExchangeRate.objects.filter(currency='USD', start_date='2020-01-03').count()
        resp = self.client.post(reverse('delete_record_url',
                                        kwargs={'slug': 'USD', 'start_date': '2020-01-03'}))
        after_delete = ExchangeRate.objects.filter(currency='USD', start_date='2020-01-03').count()
